...
const createCollege = async function (req, res) {
    try {
        ...
        if (!regname.test(name.trim())) return res.status(400).send({ status: false, message: "plese provide the name only in string & without space" })
        ...
        if (!regfname.test(fullName.trim())) return res.status(400).send({ status: false, message: "plese provide fullname in string, only single space allowed " })
